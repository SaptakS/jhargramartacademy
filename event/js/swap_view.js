...
Event.observe(document,"keydown",this._boundKeyDown)}this.__addEventListeners()
},__addEventListeners:function(){if(typeof(this.listenForEvent)==="function"){var b=function(a){this.stopMovieIfItsPlaying(a)
}.bind(this);this.selectSectionFromEventHandler=this.selectSectionFromEvent.bind(this);
this.listenForEvent(AC.ViewMaster,"ViewMasterSelectSectionWithIdNotification",true,this.selectSectionFromEventHandler);
this.listenForEvent(AC.ViewMaster,"ViewMasterWillShowNotification",true,b);this.listenForEvent(document.event,"replayMovie",false,this.stopMovieIfItsPlaying.bind(this));
if(this.options.parentSectionId){this.listenForEvent(AC.ViewMaster,"ViewMasterWillCloseNotification",false,function(a){var d=a.event_data.data;
if(this===d.sender){return}if(d.outgoingView&&d.outgoingView.id===this.options.parentSectionId){this.willClose(this.view,this.currentSection)
...
if(d>0){this.view.view().style.height=d+"px";this._heightSet=true}return d},__zIndex:1001,__manageZ:function(m){if(this.options.manageZ===true||typeof this.options.manageZ==="number"){var j="";
var h;var l;var k;var g;if((m!==true)){j=(typeof this.options.manageZ==="number")?this.options.manageZ:this.__zIndex
}if((g=this.view.view())){j=((m!==true)&&(h=parseInt(g.getAttribute("data-manage-z")))&&isNaN(h)===false)?h:j;
g.style.zIndex=j}if(this.previousSection&&this.previousSection.content){l=((m!==true)&&(l=this.previousSection.getZIndexFromContent()))?l:j;
this.previousSection.content.style.zIndex=l}if(this.currentSection&&this.currentSection.content){k=((m!==true)&&(k=this.currentSection.getZIndexFromContent()))?k:j;
...
$(e.content).removeClassName("hidden")}if(e){this._addGalleryContentClassName(e.content);
return e.willShow(this)}return null},willClose:function(d,c){if(this.delegate&&typeof(this.delegate.willClose)==="function"){this.delegate.willClose(this,this.previousSection,this.currentSection)
...
}else{this.currentSection.stopMovie()}}}}}},didShow:function(d,f,e){if(e){this._addGalleryContentClassName(e.content);
this.hideSwapViewLinks(e)}this.__manageZ(true);if(this.currentSection){this.currentSection.didShow(this)
...
}l=h.relatedElements();for(var g=0,m;(m=l[g]);g++){m.addClassName("active")}}},_addGalleryContentClassName:function(b){if(this.options.addGalleryContentClassName===true){if(b&&Object.isElement(b)){b.addClassName("gallery-content")
...
Event.observe(document,"keydown",this._boundKeyDown)}if(this.options.useTouchEvents===true&&typeof window.ontouchstart!=="undefined"){this.__touchLoadEventDependencies()
}this.__addEventListeners();this._addGalleryContentClassNameToAllSections()},__touchLoadEventDependencies:function(){if(typeof Element.trackTouches==="function"){this.__touchInitTrackTouches()
...
}}},_addGalleryContentClassNameToAllSections:function(){if(this.options.addGalleryContentClassName===true){var b;
for(b=0;b<this.orderedSections.length;b++){this._addGalleryContentClassName(this.sectionWithId(this.orderedSections[b]).content)
...
}var q=o.getAttribute("href");if(!q.startsWith("http")&&!q.startsWith("http://www.apple.com/")){var k=q;
...
}}return this.transport.responseXML};if(typeof AC.Retina!=="undefined"){AC.Retina.SwapView=function(){var b;
if(AC.Retina.sharedInstance()===null){return false}b=function(e){if(e.event_data.data.incomingView&&Object.isElement(e.event_data.data.incomingView.content)){var f=e.event_data.data.incomingView.content;
var a=e.event_data.data.sender.view.view();AC.Retina.sharedInstance().replace(f,a)
}};b=b.bindAsEventListener(this);if("Listener" in Event&&typeof AC.ViewMaster!=="undefined"){Event.Listener.listenForEvent(AC.ViewMaster,"ViewMasterDidShowNotification",false,b)
}};Event.observe(window,"load",function(){window.setTimeout(AC.Retina.SwapView,20)
})};